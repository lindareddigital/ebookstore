...
useEffect(() => {
  if (menu && slug) {
    ...
    // console.log("54", matchedItem, slug);
...
useEffect(() => {
  ...
  if (matchedMenuItem && matchedMenuItem.type === "product_by_category") {
    ...
    // console.log("categoryIds", matchedMenuItem, categoryIds);
...
const getProductsByCategory = async (categoryIds) => {
  ...
  try {
    ...
    console.log("Products by category:", books);
    setProducts(books?.result?.product);
    console.log(
      "products",
      products,
      books?.result?.product_aggregated[0]?.count?.id
    );
    
    setProductTotalCount(books?.result?.product_aggregated[0]?.count?.id);
  } catch (error) {
